  ...
  return (
    <div className="relative mt-2">
      ...
      <Link className="block" href={item.url}>
        <div
          className={`group py-3 pl-2 pr-3 rounded-xl transition-colors hover:bg-n-3/75 dark:hover:bg-n-5 ${
            active && "bg-n-3/75 dark:bg-n-5"
          }`}
        >
          <div className="flex justify-between">
          <div className="base1 text-sm font-semibold dark:text-n-1">{item.title}</div>
          <div className="base1 text-xs font-semibold dark:text-green-500">{item.price}</div>
          </div>
